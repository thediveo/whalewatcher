...
var _ = Describe("CRI API", Ordered, func() {
	...
	const withSandboxEvents = false
	//const criAPIEndpoint = "run/containerd/containerd.sock"
	const criAPIEndpoint = "/run/crio/crio.sock"
	...
	BeforeAll(func(ctx context.Context) {
		...
		By("spinning up a containerd Docker container, courtesy of the KinD k8s sig")
		...
		cdcntr := Successful(pool.BuildAndRunWithBuildOptions(
		...
		DeferCleanup(func() {
			...
			Expect(pool.Purge(cdcntr)).To(Succeed())
		})

		By("waiting for the containerized containerd to become responsive")
		Expect(cdcntr.Container.State.Pid).NotTo(BeZero())
		endpoint := fmt.Sprintf("/proc/%d/root%s",
			cdcntr.Container.State.Pid, criAPIEndpoint)
	...
	FIt("watches", func(ctx context.Context) {
