...
[![License](https://img.shields.io/github/license/thediveo/whalewatcher)](https://img.shields.io/github/license/thediveo/whalewatcher)
![build and test](https://github.com/thediveo/whalewatcher//actions/workflows/buildandtest.yaml/badge.svg?branch=master)
...
🔭🐋 `whalewatcher/v2` is a Go module that relieves applications from the tedious
...
The `v2` fortunately doesn't introduce any API changes; instead, it signals that
an important dependency has changed from the `docker/docker` Go client to the
`moby/moby/client`. This means a hearty "good riddance!" to that Docker's choice
of a brittle [CalVer](https://calver.org/) "calendar versioning" without any
`go.mod` brought all the past years. In the background, we've also upgraded to
the now-current containerd v2 Go client, as v1 is on life-support.
...
processes and later die, keeping track of only the "alive" containers.
Additionally, it tracks CRI containers when the CI EventPLEG API extension is
enabled.

On purpose, `whalewatcher` focuses solely on _running_ and _paused_ containers,
so those only that have at least an initial container process running (and thus
a PID).

Thus, one main use case for `whalewatcher` are container-aware tools that
seemingly randomly need the current state of affairs for all running containers
– such as [lxkns](https://github.com/thediveo/lxkns). These tools themselves now
don't need anymore to do the ugly lifting of container engine event tracking,
engine state resynchronization after reconnects, et cetera. Here, the
`whalewatcher` module reduces system load especially when state is requested in
bursts, as it offers a load-optimized kind of "cache". Yet this cache is always
closely synchronized to the container engine state.


The other main use case are tools that just want the start/stop events (and
pause/continue events where available) and otherwise be shielded from the gory
details of the three different APIs. For this, API consumers can request a
lifecycle event stream from a `watcher.Watcher`.
...
- supports multiple types of container engines:
  ...
  - Podman: 
    ...
    - please notice how this shows the absurdity of the podman claim that it's
      especially secure because "there's no demon" and "podman only runs when
      needed" – well, you need it obviously all the time, for workload tracking,
      for observability, for you-name-it.
